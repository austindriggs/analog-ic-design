...
% Define Ohmic to Sat Transition Values for Vds (You provided these)
Vds_transitions = [(1.4 - Vth), (1.7 - Vth), (2 - Vth)]; % Vds values for transitions
...
% Initialize Isat array.  We'll store one Isat value for each curve.
Isat_values = zeros(1, size(nfet_ds, 2)-1); % -1 because the first column is Vd

% Store plot handles
plot_handles = gobjects(1, size(nfet_ds, 2)-1);

% Plot original drain sweeps and find Isat
for i = 2:size(nfet_ds,2)
    plot_handles(i-1) = plot(nfet_ds(:,1), nfet_ds(:,i));  % Original drain sweep curve
    % Find Isat for this curve:
    Isat_values(i-1) = max(nfet_ds(saturation_region, i));  % Max current in saturation
end

% Now we will find and plot the transition points for each curve (I4, I5, I6)
transition_points = []; % Store all the transition points

for i = 1:length(Vds_transitions)
    % Get the transition Vds value for this curve
    transition_Vds = Vds_transitions(i);
    
    % For each curve (I4, I5, I6) specifically, find the closest Vds value and its corresponding Id
    curve_index = i + 4;  % Adjusted to target I4, I5, and I6 (i+4 => I4=5, I5=6, I6=7)
    
    % Find the closest Vds value and its corresponding Id for the chosen curve
    [~, idx_trans] = min(abs(nfet_ds(:,1) - transition_Vds));  % Find the closest Vds value
    transition_Vds_actual = nfet_ds(idx_trans, 1);  % Actual Vds value
    transition_Id_actual = nfet_ds(idx_trans, curve_index);  % The corresponding Id value for the selected curve (I4, I5, or I6)
    
    % Store the transition points (Vds, Id)
    transition_points = [transition_points; transition_Vds_actual, transition_Id_actual];
    
    % Plot the transition point on this curve
    plot(transition_Vds_actual, transition_Id_actual, 'ro', 'MarkerFaceColor', 'r'); % Red circle with filled color
    
    % Annotate the point with its coordinates
    text(transition_Vds_actual + 0.05, transition_Id_actual, ...
        sprintf('(%0.3f, %0.3e)', transition_Vds_actual, transition_Id_actual), ...
        'FontSize', 10, 'Color', 'r');
end

% Add the origin as the first point
transition_points = [[0, 0]; transition_points];  % Add (0, 0) at the start

% Create a smooth curve that goes through the origin and the transition points
Vds_all = transition_points(:,1);  % All Vds points
Id_all = transition_points(:,2);  % All Id points

% Use spline interpolation to create a smooth curve
Vds_interp = linspace(min(Vds_all), 1.8, 500);  % Extend Vds_interp range to reach higher Vds (e.g., 1.8)
Id_interp = spline(Vds_all, Id_all, Vds_interp);  % Spline interpolation for Id values

% Plot the spline curve (smooth curve)
plot(Vds_interp, Id_interp, 'r-', 'LineWidth', 2);  % Red smooth curve

% Set the x and y axis limits
xlim([0 3.5]);
ylim([0 6e-4]);

% Add "Ohmic" and "Saturation" labels
h_ohmic = text(0.1, 5.5e-4, 'Ohmic Region', 'Color', 'r', 'FontSize', 12, 'FontWeight', 'bold', 'ButtonDownFcn', @(src, event) move_text(src));
h_saturation = text(1.6, 5.5e-4, 'Saturation Region', 'Color', 'r', 'FontSize', 12, 'FontWeight', 'bold', 'ButtonDownFcn', @(src, event) move_text(src));
...
% Display the Isat values
fprintf('\nIsat values for each drain sweep:\n');
disp(Isat_values);

% Create a legend with Isat values ONLY for curves 4, 5, and 6
legend_indices = [6, 5, 4]; % Reordered indices

% Get corresponding plot handles in the new order
legend_handles = plot_handles(legend_indices);

% Generate legend entries in the new order
legend_entries = arrayfun(@(k) sprintf('I_{sat} = %.2e A', Isat_values(k)), legend_indices, 'UniformOutput', false);

% Apply the reordered legend
legend(legend_handles, legend_entries, 'Location', 'northoutside');

% Move text function for interactivity
function move_text(src)
    % Callback for dragging text labels
    set(gcf, 'WindowButtonMotionFcn', @dragging);
    ...
    function dragging(~, ~)
        ...
        set(src, 'Position', [cp(1,1), cp(1,2)]);
    end
...
title('nFET Gate Sweep - Square Law Region');
grid on;
hold on;
...
% Given kappa value (example value, change as needed)


% Get the slope from the linear fit (p_sqrt_fit(1))
slope = p_sqrt_fit(1);
...
fprintf('\nProcess Transconductance Parameter (K): %.4e A/V^2\n', K);
...
%% Above-Threshold Drainsweep Plot Traced To Find Early Voltage
figure;
hold on;
...
colors = lines(3); % Using 3 distinct colors for 3 cases

% Initialize arrays to store plot handles and legend entries
plot_handles = [];
legend_entries = {};

% Plot above-threshold drain sweeps
fprintf('\nNegative Early Voltage Values From Tracing (Above Threshold)\n');
for i = 5:7  % Process columns 5 to 7
    color_idx = i - 4; % Adjust index for color mapping
    
    % Plot the original drain sweep curve
    h_data = plot(nfet_ds(:,1), nfet_ds(:,i), 'Color', colors(color_idx, :), 'LineWidth', 1.5);  
    
    % Extract data points above Vth (0.65V) for curve tracing
    if i == 7
        idx = (nfet_ds(:,1) > 1);
    else
        idx = (nfet_ds(:,1) > Vth);  % Identify the region where Vd > Vth
    end
    Vd_above = nfet_ds(idx,1);   % Extract corresponding Vd values
    Id_above = nfet_ds(idx,i);   % Extract corresponding Id values
    
    % Perform linear regression (fit a line) on the selected region
    coeffs = polyfit(Vd_above, Id_above, 1); % Linear fit (y = mx + b)
    slope = coeffs(1);
    intercept = coeffs(2);
    ...
    Vd_ext = linspace(VA, max(Vd_above), 100); % Extend from VA to max Vd
    Id_ext = polyval(coeffs, Vd_ext); % Compute corresponding Id values
    
    % Plot the extended fitted line in matching color
    h_fit = plot(Vd_ext, Id_ext, '--', 'Color', colors(color_idx, :), 'LineWidth', 1.5); 
    
    % Add handles and legend entries immediately after plotting
    plot_handles = [plot_handles, h_data, h_fit];  
    legend_entries = [legend_entries, ...
        sprintf('Case %d Data', color_idx + 3), ...  % Adjust case number to 4, 5, 6
        sprintf('Case %d Fit (V_A = %.2f V)', color_idx + 3, VA)];  % Adjust case number to 4, 5, 6
    
    % Print VA value to console
    fprintf('-V_A for I_d%d: %.2f V\n', i, VA);
end

% Plot the y=0 horizontal line in bold
plot(xlim, [0 0], 'k-', 'LineWidth', 2);  % y=0 line (horizontal), bolded

% Set grid on
grid on;

% Set title, axis labels, and other properties
title('Above Threshold Drainsweep and Early Voltage Fit');
xlabel('Drain Voltage (V)');
ylabel('Drain Current (A)');

% Set legend with updated entries
...
% Filter data for Vgs > 0.8V
valid_idx_p = pfet_gs(:,1) < 2.5; 
Vgs_filtered_p = pfet_gs(valid_idx_p, 1) - 3.3;
sqrt_Id_filtered_p = sqrt(pfet_gs(valid_idx_p, 2));

% Plot the square root of drain current vs gate voltage as Vsg
figure;
plot(Vgs_filtered_p, sqrt_Id_filtered_p, 'o', 'DisplayName', 'Filtered Data');
xlabel('Gate Voltage (V)');  % x-axis label updated to Vsg
ylabel('Square Root of Drain Current (A^{1/2})');
title('pFET Gate Sweep - Square Law Region');  % Title updated
grid on;
hold on;

% Fit a line to the filtered data
p_sqrt_fit = polyfit(Vgs_filtered_p, sqrt_Id_filtered_p, 1);
sqrt_fit_line = polyval(p_sqrt_fit, Vgs_filtered_p);

% Plot the fitted line
plot(Vgs_filtered_p, sqrt_fit_line, 'r-', 'LineWidth', 2, 'DisplayName', 'Linear Fit');

% Find the x-intercept (threshold voltage)
Vth_p = -p_sqrt_fit(2) / p_sqrt_fit(1); % x-intercept formula = about 0.65V

% Extend the fit line to x-axis
Vgs_extend = linspace(min(Vgs_filtered_p), Vth_p, 100);
sqrt_fit_extend = polyval(p_sqrt_fit, Vgs_extend);
plot(Vgs_extend, sqrt_fit_extend, 'b--', 'LineWidth', 1, 'DisplayName', 'Extended Fit');

% Mark the threshold voltage on the plot
xline(Vth_p, 'k--', 'LineWidth', 1, 'Label', sprintf('V_{th} = %.3f V', Vth_p));

legend;
hold off;

% Display threshold voltage
fprintf('\npFET Threshold Voltage (Vth): %.4f V\n', Vth_p);

% Given kappa value (example value, change as needed)

% Get the slope from the linear fit (p_sqrt_fit(1))
slope_o = p_sqrt_fit(1);

% Calculate K from the slope
K_p = 2 * kappa_p * slope_o^2;

% Display the value of K
fprintf('\npFET Process Transconductance Parameter (K): %.4e A/V^2\n', K_p);
